...
### 14.6 新算法 TODO：DA3D / BM4D（2026-09-11）

以下为新增实现计划，全部待办；接口名为草案，先建立 CPU 参考路径，再接入插件和优化。建议顺序为 DA3D → BM4D，各自独立验收。

#### DA3D：由引导图驱动的末级降噪

参考：[Pierazzo / Facciolo，IPOL 2017，论文与参考源码](https://www.ipol.im/pub/art/2017/203/)。DA3D 输入原始噪声图及已有降噪结果，在形状与数据自适应 patch 上进行频域收缩，作为现有降噪器的后处理。

- [ ] **参考与合同**：固定 IPOL 论文、源码版本及哈希，核对源码许可证；逐项整理 patch 选择、自适应权重、频域收缩、聚合和边界规则，建立可重跑的参考输出。
- [ ] **接口草案**：规划 `nss.DA3D(clip, ref, sigma, planes)`，`clip` 为原始噪声图，`ref` 为必需的降噪引导图；定义尺寸、格式、帧数匹配、sigma 单位、零 sigma 和未处理平面的行为。首版逐帧处理，明确灰度及多平面的支持边界。
- [ ] **CPU 核**：先实现可对拍的标量路径，包括引导图驱动的自适应 patch、动态采样、频域收缩、逆变换及加权归一化；核对论文的大 patch（64×64）路径。变换类型和归一化以参考为准，不直接用现有 BM3D DCT 代替。
- [ ] **插件接入**：新增独立 DA3D 核与 host 入口，复用格式转换、平面选择和有界 workspace；对接 BM3D/NCSR 等产生的 `ref`，验证双输入帧依赖、并发确定性、边界覆盖和零聚合权重处理。
- [ ] **质量验收**：固定相同 noisy/ref/sigma 对拍 IPOL，记录逐像素误差及 PSNR/SSIM；比较引导图与 DA3D 输出，覆盖平坦渐变、纹理、边缘、极小图和不同噪声强度，保留质量回退样本，不预设所有引导器均获益。
- [ ] **性能与交付**：正确性通过后再做 Highway/变换及 scratch 优化；同机配对记录 DA3D 增量耗时、完整“引导器 → DA3D”耗时和峰值内存，补齐参数文档、脚本示例及回归测试后再标记完成。

#### BM4D：三维块分组与四维协同滤波

参考：[Maggioni 等，TIP 2013，Nonlocal Transform-Domain Filter for Volumetric Data Denoising and Reconstruction](https://doi.org/10.1109/TIP.2012.2210725)；[作者软件与论文目录](https://webpages.tuni.fi/foi/GCF-BM3D/index.html)。BM4D 将相似三维体块堆叠为四维组；将视频帧映射为体数据是本项目的适配，不能称为带运动轨迹构造的 V-BM4D。

- [ ] **版本与范围**：冻结所复现的 BM4D 论文/参考版本及参数，区分经典算法和后续相关噪声扩展；先做实值 AWGN，Rician、复数重建与相关噪声另行排期。核对作者软件的使用及再分发许可，确定参考对拍与独立实现的边界。
- [ ] **体数据与接口合同**：规划 `nss.BM4D`，先定义 VS 帧序列到 `(x, y, z)` 体数据的映射、体块深度、搜索范围、组大小及 Basic/Final 引导输入；冻结首尾帧、短 clip、场景切换与平面处理规则。公开签名待这些合同确定后再注册。
- [ ] **CPU 基线**：实现三维体块 SSD 搜索、稳定分组、体块打包、四维可分离变换、Basic hard-threshold、Final Wiener 和重叠体块加权重建；逐阶段核对变换尺度、噪声方差和聚合权重。已有二维 matcher/pack 仅复用经确认适用的原语，不将 BM3D 的时间搜索直接当作 BM4D。
- [ ] **VS 调度与内存**：推导 Basic→Final 及所有贡献块的完整帧依赖，设计有界体数据 workspace、缓存与确定性归约；评估现有 `VAggregate` 协议能否表达体块贡献，不能直接套用二维 patch 布局。验证随机取帧、多线程、窗口接缝和内存上限。
- [ ] **正确性与质量验收**：先用固定三维合成体及作者参考对拍两阶段输出，再测试静止、运动、遮挡和切镜视频；记录数值误差、PSNR/SSIM、时间一致性及拖影。将整卷参考与有界视频窗口的差异单独列为工程适配，不混作论文复现通过。
- [ ] **优化与交付**：基线通过后再评估 Highway 三维距离、变换批处理和体块复用；同机配对报告完整 Basic→Final 耗时、峰值内存及各体块/搜索配置的质量取舍。补齐格式与参数回归、文档及示例；CUDA/Vulkan 后续独立排期。
