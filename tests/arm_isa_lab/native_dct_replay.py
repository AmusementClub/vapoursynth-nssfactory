...
import argparse,json,hashlib,subprocess,sys,platform
...
def run(a):
 ...
 kw=cfg['kwargs']
 if cfg['stage']!='basic' or kw.get('radius',0)!=0:raise ValueError('Basic spatial replay only')
 w,h=cfg['size'];b=kw['block_size'];g=kw['group_size'];step=kw['block_step']
 raw=np.fromfile(cfg['sample'],dtype=np.uint8).reshape(1080,1920)
 clean=raw[np.arange(h)*1080//h][:,np.arange(w)*1920//w].astype(np.float32)/np.float32(255)
 ...
 for side in ('baseline','candidate'):
  ...
  cmd=(['taskset','-c',str(a.cpu)] if platform.system()=='Linux' else [])+[str(getattr(a,side)),str(input_file),str(w),str(h),str(b),str(g),str(step),str(sigma),str(px),str(py)]
  with log.open('w') as out:subprocess.run(cmd,stdout=out,check=True)
 classification=a.out/'classification.json'
 subprocess.run([sys.executable,str(Path(__file__).resolve().parents[1]/'bm3d_classify_replay.py'),'--baseline',str(logs[0]),'--candidate',str(logs[1]),'--out',str(classification)],check=True,stdout=subprocess.DEVNULL)
