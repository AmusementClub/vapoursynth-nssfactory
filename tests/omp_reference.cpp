...
int lssc_omp_workspace_reference(const float* y, int m, const float* D, int atoms, int ldd, int sparsity, float* a,
...
int lssc_omp_reference(const float* y, int m, const float* D, int atoms, int ldd, int sparsity, float* a) {
    if (!y || !D || !a || m < 1 || atoms < 1 || ldd < m) return 0;
    std::fill_n(a, atoms, 0.f);
    if (sparsity < 1) return 0;
    const int need = lssc_omp_work_floats(m, atoms, sparsity);
    float scratch[3008];
    if (need <= 3008) return lssc_omp_workspace_reference(y, m, D, atoms, ldd, sparsity, a, scratch, 3008);
    ResourceVector<float> dynamic(need);
    return lssc_omp_workspace_reference(y, m, D, atoms, ldd, sparsity, a, dynamic.data(), need);
}


void lssc_denoise_plane_reference(const float* src, int width, int height, int sstride, float* num, float* den, int buf_stride,
